...
export class Cards {
  ...
  checkAll() {
    ...
    const audioArr = Array.from(this.files.audio!).map((it) => it.name.slice(0, it.name.indexOf(".")));
    const imagesArr = Array.from(this.files.images!).map((it) => it.name.slice(0, it.name.indexOf(".")));

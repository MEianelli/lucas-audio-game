...
import { CheckIPAttempts } from "@/lib/apis/ipAttempt/ipAttempt";

async function Register(req: NextApiRequest, res: NextApiResponse) {
  const forwarded = req.headers["x-forwarded-for"];
  const ip = typeof forwarded === "string" ? forwarded.split(",")[0].trim() : req.socket.remoteAddress;
  const checkIp = await CheckIPAttempts(ip);
  if (!checkIp) {
    return res.status(200).json({ res: "tooManyRequests" });
  }
  ...
  if (data?.id) {
    ...
    const { data, error } = await supabase.from("users").insert({ name, pass }).select().single();
